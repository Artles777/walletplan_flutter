...
- Перед добавлением новой логики сначала изучи текущую реализацию в связанных
  файлах и убедись, что понимаешь уже существующий архитектурный паттерн
  модуля.
...
- Не создавай `StatefulWidget` по умолчанию. Если виджету нужно состояние,
  оно должно жить в `use_*` store / composable hook, а сам виджет должен
  оставаться `CompositionWidget` или `StatelessWidget` как слоем биндинга UI.
...
- Не превращай UI-компонент в локальный state-container через
  `StatefulWidget`, если ту же задачу можно решить через существующий store
  паттерн на `flutter_compositions`.
- Соблюдай правило: один файл `*.widget.dart` - один основной компонент.
  Приватные вспомогательные виджеты внутри файла допустимы, если они
  небольшие, локальны для этого компонента и не переиспользуются в нескольких
  местах. Если виджет становится самостоятельной частью UI или начинает
  повторяться, выноси его в отдельный `*.widget.dart`.
...
- Перед изменением логики не придумывай решение с нуля: сначала проверь,
  как похожая задача уже решена в текущем модуле или соседних частях проекта,
  и только потом достраивай решение в том же стиле.
